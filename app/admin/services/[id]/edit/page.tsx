...
import { getServiceTypeByIdAction, updateServiceTypeAction, uploadMediaAction } from "@/app/admin/services/actions";
...
export default function EditServicePage({ params }: EditServicePageProps) {
  ...
  useEffect(() => {
    const fetchService = async () => {
      try {
        const service = await getServiceTypeByIdAction(serviceId);
        setFormData({
        ...
        if (service.icon_url) {
          setIconPreview(service.icon_url.startsWith("http") ? service.icon_url : `${API_BASE_URL}${service.icon_url}`);
        }
      } catch (error) {
        console.error("Failed to fetch service:", error);
        toast.error(t.errorFetch || "Failed to fetch service");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await updateServiceTypeAction(serviceId, formData);
      toast.success(t.form.successUpdate);
      router.push("/admin/services");
      router.refresh();
    } catch (error) {
      console.error("Failed to update service:", error);
      toast.error(t.form.errorUpdate);
    } finally {
      setLoading(false);
    }
  };
