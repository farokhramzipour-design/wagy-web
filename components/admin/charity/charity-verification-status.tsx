...
interface CharityVerificationStatusProps {
  status: VerificationStatusResponse;
  phoneVerified: boolean;
}

export function CharityVerificationStatus({ status, phoneVerified }: CharityVerificationStatusProps) {
  const router = useRouter();
  const { lang } = useLanguage();
  const t = content[lang].becomeSitter;
  const tCharity = t.charityVerification;
  ...
  const steps = useMemo(() => {
    return [
      {
        id: 1,
        title: tSteps.address.title,
        description: tSteps.address.description,
      ...
      {
        id: 2,
        title: tSteps.phone.title,
        description: tSteps.phone.description,
        completed: phoneVerified,
        pending: false,
      },
      {
        id: 3,
        title: tSteps.identity.title,
        description: tSteps.identity.description,
        completed: status.national_code?.added && status.shahkar?.verified,
        pending: false,
      },
      {
        id: 4,
        title: tSteps.documents.title,
        description: tSteps.documents.description,
        completed:
  ...
  const showGlobalPending = step1Pending && step4Pending;

  const handleStartStep = (stepId: number) => {
    switch (stepId) {
      case 1:
        router.push("/app/charity/create/address");
        break;
      case 2:
        router.push("/app/charity/create/phone");
        break;
      case 3:
        router.push("/app/charity/create/identity");
        break;
      case 4:
        router.push("/app/charity/create/documents");
        break;
    }
  };
